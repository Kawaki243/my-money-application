...
export const addThousandsSeparator = (num) => {
    ...
    // Regex for Vietnam numbering system
    // It handles the first three digits, then every three digits
    const lastThree = integerPart.substring(integerPart.length - 3);
    const otherNumbers = integerPart.substring(0, integerPart.length - 3);

    if (otherNumbers !== '') {
        // Apply comma after every three digits for the 'otherNumbers' part
